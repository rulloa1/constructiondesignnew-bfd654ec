...
export const VideoHero: React.FC<VideoHeroProps> = ({
  ...
  return <section className="relative h-screen w-full overflow-hidden">
      ...
      <div className="absolute inset-0">
        <video autoPlay muted loop playsInline onLoadedData={() => setVideoLoaded(true)} className={`w-full h-full object-cover transition-opacity duration-1000 ${videoLoaded ? 'opacity-100' : 'opacity-0'}`}>
      ...
      <div className="relative h-full flex flex-col items-center justify-center px-6 lg:px-12">
        ...
        <div className="opacity-0 animate-fade-in delay-500">
          <Button onClick={onOpenPortfolio} size="lg" className="bg-gold hover:bg-gold/90 text-charcoal font-inter font-semibold px-10 py-7 text-lg tracking-wider uppercase transition-all duration-300 hover:scale-105 hover:shadow-2xl hover:shadow-gold/30 rounded-sm">
            View Portfolio
          </Button>
        </div>
